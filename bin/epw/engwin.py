#! /usr/bin/env python3
#目的：处理vasp计算得到的本征能量文件EIGENVAL或qe计算得到的本征能量文件prefix.xml
#获得指定能带能量范围：python3 engwin.py *.xml e 能带号
#确定给定能量范围内的能带数： python3 engwin.py *.xml n 能量范围
    ...
    if xml_name == "EIGENVAL":
        ...
        for i in entries:
            ...
            for line in content[i+1:]:
                if len(line.split()) == 3:   # %2->3,that could be succussfully run with my file.
