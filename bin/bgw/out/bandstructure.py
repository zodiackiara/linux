...
# 目的：读取bandstructure.dat文件绘制能带结构图，输出带隙值band.gap文件
...
with open ('band.gap', 'w') as gap:
    print('LDA direct gap: {:.3f} eV'.format(np.amax(elda[nv]) - np.amax(elda[nv-1])),file=gap)
    print('LDA indirect gap: {:.3f} eV'.format(np.amin(elda[nv]) - np.amax(elda[nv-1])),file=gap)
    print('GW direct gap: {:.3f} eV' .format(np.amax(eqp[nv]) - np.amax(eqp[nv-1])),file=gap)
    print('GW indirect gap: {:.3f} eV' .format(np.amin(eqp[nv]) - np.amax(eqp[nv-1])),file=gap)
...
k_special_index = np.array([0, 50, 100, 150])
k_special_label = np.array(['G', 'M', 'K', 'G'])
...
plt.title('bgw bandstructure')
plt.savefig('bgw_band.png', bbox_inches='tight')